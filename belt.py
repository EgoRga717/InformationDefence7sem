...
def decryption(encrypted_text = b"", key = b""):
    ...
    result = b""
    for i in range(chunks_count):
        chunk = encrypted_text[chunk_encrypt_size * i:chunk_encrypt_size * (i + 1)]
        result += tmp.decryption(chunk)
    return result
